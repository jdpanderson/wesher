...
func statusFixture() (*wg.Report, map[string]peerInfo, time.Time) {
	...
	return r, map[string]peerInfo{"KEYB": {Name: "b", Identity: &identityB, Overlay: netip.MustParseAddr("10.0.0.2")}}, now
}
...
func Test_renderStatusJSON(t *testing.T) {
	...
	assert.Equal(t, "10.0.0.2", got.Peers[0].Overlay)
	assert.Equal(t, "192.0.2.2:51820", got.Peers[0].Endpoint)
	assert.Empty(t, got.Peers[1].Name)
	assert.Empty(t, got.Peers[1].Overlay, "unknown to the cluster state")
}
