...
| `--bind-addr ADDR` | WESHER_BIND_ADDR | IP address to bind to for cluster membership (cannot be used with --bind-iface) | first public IPv4, else memberlist picks a private one |
| `--bind-iface IFACE` | WESHER_BIND_IFACE | interface to bind to for cluster membership; its first global unicast IPv4 address is used (cannot be used with --bind-addr) |  |
